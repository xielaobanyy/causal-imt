...
from torchvision.transforms import Compose, Resize, CenterCrop, ToTensor, Normalize, RandomHorizontalFlip, GaussianBlur, ColorJitter
...
class FashionIQDataset(Dataset):
    ...
    def __init__(self, split: str, dress_types: List[str], mode: str, preprocess: callable, label_mapping: dict):
        """
        ...
        for dress_type in dress_types:
            if dress_type not in ['bifur']:
                raise ValueError("dress_type should be in ['bifur']")
    ...
    def __getitem__(self, index):
        try:
            if self.mode == 'relative':
                ...
                image_captions_cls = self.triplets[index]['captions_cls']
                ...
                if self.split == 'train':
                    ...
                    label_name = self.triplets[index]['label']  # Assumes a "label" field exists
                    label = self.label_mapping[label_name]


                    return reference_image, target_image, image_captions, label, image_captions_cls
                    ...
                    label_name = self.triplets[index]['label']  # Assumes a "label" field exists
                    label = self.label_mapping[label_name]
                    return reference_image, target_image, image_captions, label
